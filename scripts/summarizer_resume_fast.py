...
OLLAMA_MODEL = "qwen3:1.7b"  # Faster model
BATCH_SIZE = 10  # Smaller batch size for memory efficiency
PROGRESS_FILE = "summarizer_progress_fast.json"
...
def check_ollama_health():
    ...
    try:
        ...
        response = requests.get(OLLAMA_HEALTH_API, timeout=10)
        ...
        if OLLAMA_MODEL not in model_names:
            return False, f"Model {OLLAMA_MODEL} not loaded. Available models: {model_names[:5]}"
        
        return True, "OK"
    except requests.exceptions.ConnectionError:
        return False, f"Cannot connect to Ollama server at {OLLAMA_HEALTH_API}"
...
def main():
    ...
    print(f"Using Ollama server: {OLLAMA_API}")
    ...
    if not is_healthy:
        ...
        print("Please check if the lab's Ollama server is running and accessible")
        return
    else:
    ...
    print("Starting fast summarization processing...")
    ...
    for batch in fetch_objects_in_batches(collection, batch_size=50):
        ...
        for i in range(0, len(objects_to_process), BATCH_SIZE):
            ...
            if consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
                ...
                print("Please check if the Ollama server is running and restart the script")
                return
